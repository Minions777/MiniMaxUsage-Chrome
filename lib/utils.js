...
(function (root, factory) {
  ...
  // The MiniMax M3 API field `current_interval_remaining_percent` (and the
  // weekly equivalent) is SEMANTICALLY REVERSED: despite its name it represents
  // "USED percent" (92 = 92% used → 8% remaining). correctRemainingPct() applies
  // the reversal AND clamps to [0,100] AND falls back to a count-based ratio when
  // the API field is absent. It NEVER returns null, so downstream consumers
  // (badge.js, display.js) can trust the resolved value without re-deriving it.
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  /**
   * Resolve the true remaining-percent from a raw M3 API value.
   * @param {number|null|undefined} rawApiPct - raw `*_remaining_percent` (used %)
   * @param {number} total - interval/weekly total count
   * @param {number} used - interval/weekly used count
   * @returns {number} corrected remaining% in [0,100] (integer)
   */
  function correctRemainingPct(rawApiPct, total, used) {
    let pct;
    if (rawApiPct != null && !isNaN(rawApiPct)) {
      pct = 100 - Number(rawApiPct); // M3 reversal
    } else {
      pct = total > 0 ? ((total - (Number(used) || 0)) / total) * 100 : 0;
    }
