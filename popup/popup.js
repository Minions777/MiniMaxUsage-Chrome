...
function applySettingsToUI(settings) {
  ...
  // Endpoint radio buttons - sync visual state with actual input checked state
  document.querySelectorAll('input[name="endpoint"]').forEach(radio => {
    const isChecked = radio.value === (settings.endpoint || 'china');
    radio.checked = isChecked;
    // Also update parent label's selected class
    const label = radio.closest('.endpoint-option');
    if (label) {
...
document.getElementById('btnSaveSettings').addEventListener('click', async () => {
  // FIX: use optional chaining + fallback to prevent null when radio not checked
  const selectedEndpoint = document.querySelector('input[name="endpoint"]:checked');
  const settings = {
    apiKey: inputAPIKey.value.trim(),
    endpoint: selectedEndpoint ? selectedEndpoint.value : (currentSettings?.endpoint || 'china'),
...
document.querySelectorAll('.endpoint-option').forEach(option => {
  option.addEventListener('click', () => {
    const radio = option.querySelector('input[type="radio"][name="endpoint"]');
    if (radio) {
      // Uncheck all options first
      document.querySelectorAll('.endpoint-option').forEach(opt => {
